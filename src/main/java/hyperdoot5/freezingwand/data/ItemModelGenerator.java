...
public class ItemModelGenerator extends ItemModelProvider {
    ...
    protected void registerModels() {
        ...
        wand(FWItems.FREEZING_WAND.getId().getPath(), prefix("item/freezing_wand/basic"))
                .override().predicate(FreezingWandItem.BASIC, 1).model(basic).end()
                .override().predicate(FreezingWandItem.ICE, 1).model(ice).end()
                .override().predicate(FreezingWandItem.PACKED_ICE, 1).model(packed_ice).end()
                .override().predicate(FreezingWandItem.BLUE_ICE, 1).model(blue_ice).end();
    }
